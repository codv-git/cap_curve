...
def capcurve(y_values, y_preds_proba):
    ...
    y_cap_df_s = y_cap_df_s.sort_values([1],
    ...
    ax.plot([percent, percent], [0.0, val], color='green', linestyle='--',
            linewidth=1)
    ax.plot([0, percent], [val, val], color='green', linestyle='--',
            linewidth=1, label=str(round(val*100, 2))
            + '% of positive obs at '
            + str(int(percent*100))+'%')

    plt.xlim(0, 1.05)
    plt.ylim(0, 1.05)
    plt.title("CAP Curve - a_r value ="+str(round(ar_value, 5)))
